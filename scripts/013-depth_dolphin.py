...
class Mission(Node):
    ...
    def __init__(self):
        ...
        self.call_switch_controller(["dolphin_controller"], ["riptide_controller", "log_controller", "depth_controller"])
    ...
    def execute_fsm(self):
        ...
        if self.state == State.FAILSAFE:
            ...
            # Current state
            msg.data = "Depth"
            self.state = State.DEPTH
            self.get_logger().info("State Depth")
            self.send_immersion_goal()

        elif self.state == State.DEPTH:
            msg.data = "Dolphin"
            self.state = State.DOLPHIN
            self.call_switch_controller(["riptide_controller", "log_controller", "depth_controller"], ["dolphin_controller"])
