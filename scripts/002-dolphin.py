...
from riptide_msgs.msg import Pressure, Multiplexer
...
class Mission(Node):
    ...
    def __init__(self):
        super().__init__('mission_dolphin')
    ...
    def multiplexer_callback(self, msg):
        if self.state == State.IDLE and msg.automatic:
    ...
    def execute_fsm(self):
        ...
        if self.state == State.FAILSAFE:
            ...
            # Current state
            msg.data = "Action 1m"
            self.state = State.ACTIONDOLPHIN
            self.get_logger().info("Calling Action 1m")

        elif self.state == State.ACTIONDOLPHIN:
            # Call 2 m action
            self.send_goal(2., self.duration)
